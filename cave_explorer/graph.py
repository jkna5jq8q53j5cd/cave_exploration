...
class Graph:
    def __init__(self, parent_logger, grid_step_size, prm_num_nodes, use_energy_costs, use_prm, show_connectivity, prm_max_edge_length, use_distance_transform_graph):
        ...
        self.marker_nodes_.scale.x = .6
        self.marker_nodes_.scale.y = .6
        self.marker_nodes_.scale.z = .6
            ...
            if not occupied:
                for i in range(x-5,x+5):
                    for j in range(y-5,y+5):
                        if (grid_img[x][y]==100):
                            count+=1
                        elif (distance_transform_map[x][y] > distance_transform_map[i][j]):
                            count+=1
                if (count >= 85):
            ...
            for node_j in self.nodes_:
                ...
                if node_i != node_j:
                    ...
                    if distance < distance_threshold:
                        ...
                        if node_i.is_connected(grid_img, node_j):
    ...
    def get_closest_node(self, xy):
        ...
        for i in range(len(self.nodes_)):
            ...
            euclidean_dist = math.sqrt((xy[0]-self.nodes_[i].x)**2+(xy[1]-self.nodes_[i].y)**2)
            # Find the node with the lowest euclidean distance
    ...
    def generate_marker_msgs(self):
        ...
        if self.groups_ == None:
            ...
            for node_i in self.nodes_:
                point = Point(y=(node_i.x*0.3)+self.origin_y, x=(node_i.y*0.3)+self.origin_x, z=0.1)
        ...
        for node_i in self.nodes_:
            for node_j_index in range(len(node_i.neighbours)):
                ...
                point = Point(y=node_i.x*0.3+self.origin_y, x=node_i.y*0.3+self.origin_x, z=0.05)
                self.marker_edges_.points.append(point)
                
                point = Point(y=node_j.x*0.3+self.origin_y, x=node_j.y*0.3+self.origin_x, z=0.05)
                self.marker_edges_.points.append(point)
...
def is_occluded(img, p1, p2, threshold=100):
    """
